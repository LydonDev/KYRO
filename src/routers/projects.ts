...
const appName: string = env.VITE_APP_NAME || "Kyro";

const router = Router();
const database = new Database(`${appName}.db`);
...
router.get("/", async (req: any, res) => {
  try {
    ...
    if (isAdmin && req.query.userId) {
      ...
      projects = await db.projects.getUserProjectsWithServerCount(
        req.query.userId,
      );
    } else {
...
router.delete("/:id", async (req: any, res) => {
  try {
    ...
    if (project.name === "Default") {
      return res
        .status(400)
        .json({ error: "Cannot delete the default project" });
    }
    ...
    const defaultProject = await db.projects.getOrCreateDefaultProject(
      project.userId,
    );
    ...
    try {
      ...
      `,
        ...
        .run(defaultProject.id, new Date().toISOString(), project.id);
