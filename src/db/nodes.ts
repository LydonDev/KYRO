...
export function createNodesRepository({ db }: DatabaseContext) {
  const repository = {
      ...
      for (const node of nodes) {
        if (node.regionId) {
          const regionRow = db
            ...
            .get(node.regionId) as any;
          if (regionRow) {
            node.region = {
              ...regionRow,
              serverLimit: regionRow.serverLimit
                ? Number(regionRow.serverLimit)
                : null,
    ...
    create: async (
      data: Omit<Node, "id" | "connectionKey" | "createdAt" | "updatedAt">,
    ): Promise<Node> => {
      const node = {
        id: randomUUID(),
