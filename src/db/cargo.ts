...
export function createCargoRepository({ db }: DatabaseContext) {
  const repository = {
    ...
    createCargo: async (
      data: Omit<Cargo, "id" | "createdAt" | "updatedAt">,
    ): Promise<Cargo> => {
      const cargo = {
        id: randomUUID(),
        ...data,
    ...
    findContainer: async (id: string): Promise<CargoContainer | null> => {
      const row = db
        .prepare("SELECT * FROM cargo_containers WHERE id = ?")
        .get(id) as any;
      return row ? parseCargoContainerRow(row) : null;
    },

    createContainer: async (
      data: Omit<CargoContainer, "id" | "createdAt" | "updatedAt">,
    ): Promise<CargoContainer> => {
      const container = {
        id: randomUUID(),
        ...data,
    ...
    deleteContainer: async (id: string): Promise<void> => {
      const result = db
        .prepare("DELETE FROM cargo_containers WHERE id = ?")
        .run(id);
      if (result.changes === 0) {
