  ...
  const repository = {
    findMany: async ({ where }: { where?: any } = {}): Promise<Region[]> => {
      const conditions = where
        ? Object.entries(where).map(([key]) => `${key} = ?`)
        : [];
    ...
    findUnique: async ({ id }: { id: string }): Promise<Region | null> => {
      ...
      if (region.fallbackRegionId) {
        const fallbackRow = db
          .prepare("SELECT * FROM regions WHERE id = ?")
          .get(region.fallbackRegionId) as any;
        if (fallbackRow) {
    ...
    findByIdentifier: async ({
      ...
      const row = db
        .prepare("SELECT * FROM regions WHERE identifier = ?")
    ...
    create: async (
      data: Omit<Region, "id" | "createdAt" | "updatedAt" | "nodes">,
    ): Promise<Region> => {
      const region = {
        id: randomUUID(),
        ...data,
    ...
    delete: async ({ id }: { id: string }): Promise<void> => {
      ...
      if (fallbackCount.count > 0) {
        throw new Error(
          "Cannot delete region that is used as a fallback by other regions",
        );
      }
    ...
    findBestNodeInRegion: async (regionId: string): Promise<string | null> => {
      ...
      const nodeServerCounts = await Promise.all(
        onlineNodes.map(async (node) => {
          const result = db
            .prepare("SELECT COUNT(*) as count FROM servers WHERE nodeId = ?")
